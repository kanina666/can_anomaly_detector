...
- **`first_pos`** — `first_pos_v2/`: whole-image ResNet18 **ИЛИ**
  ...
  HSV-детектором блика). См. `first_pos_v2/model.py` за деталями.
...
### Скорость (замерено на CPU ноутбука)
...
Ни разу не измерено на настоящем Raspberry Pi 5. ARM Cortex-A76
 заметно слабее CPU ноутбука x86. DINOv2 ViT-S/14 — самая тяжёлая
